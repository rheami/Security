...
for vm_opts_file in "$IDE_BIN_HOME/pycharm$BITS.vmoptions" "$HOME/.PyCharm40/pycharm$BITS.vmoptions" "$PYCHARM_VM_OPTIONS"; do
...
if [ "$IS_EAP" = "true" ]; then
  ...
  if [ -r "$IDE_BIN_HOME/lib$AGENT_LIB.so" ]; then
    AGENT="-agentlib:$AGENT_LIB=disablealloc,delay=10000,sessionname=PyCharm40"
  fi
fi
...
LD_LIBRARY_PATH="$IDE_BIN_HOME:$LD_LIBRARY_PATH" "$JDK/bin/java" \
  ...
  -Djb.restart.code=88 -Didea.paths.selector=PyCharm40 \
